...
void setup() {
  ...
  movers2 = new ArrayList();
  for (int i = 0; i < botCount; i++) {
    movers2.add(new Mover(new PVector(i*20, canvas.pgHeight/2 + sin(i)*15 ), 1, random(1, 5)));
  }
}
...
void draw() {
  ...
  for (int i = movers2.size()-1; i >= 0; i--) {
    Mover mover = (Mover) movers2.get(i);
    mover.run(movers2, canvas);
  }
  /*
  for (int i = Movers.size()-1; i >= 0; i--) {
    Mover mover = (Mover) Movers.get(i);
  ...
  */
  ...
  for (int i = movers2.size()-1; i >= 0; i--) {
    Mover mover = (Mover) movers2.get(i);
    for (int k = movers2.size()-1; k >= 0; k--) {
      if(i != k){
        Mover other = (Mover) movers2.get(k);
        mover.checkCollision(other);
      }
    }
  }
